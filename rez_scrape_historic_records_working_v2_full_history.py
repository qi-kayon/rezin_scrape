# -*- coding: utf-8 -*-
"""Improved Rez_Scrape_Historic_Reco

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/10-6ZmSsSyS80LFyqDC7wgvLNGPQoore5
"""
    ...
    for attempt in range(max_retries):
        try:
            ...
            if response.status_code == 200:
                print(f"Successfully downloaded PDF from {url} (Attempt {attempt + 1})")
                return io.BytesIO(response.content)
            else:
                print(f"Failed to download PDF from {url}. Status code: {response.status_code}")
        except requests.exceptions.RequestException as e:
            print(f"Error downloading PDF (Attempt {attempt + 1}): {e}")
...
def try_download_with_fallback(report_date_str):
    year = report_date_str[:4]
    # Extract MMDD and YY (last 2 digits of the year)
    date_part = report_date_str[5:].replace('-', '')  # MMDD
    year_short = year[-2:]  # YY

    # Define all 4 URL combinations
    url_formats = [
        f"https://knowthefactsmmj.com/wp-content/uploads/ommu_updates/{year}/{date_part}{year_short}-OMMU-Update.pdf",  # MMDDYY format with -OMMU-Update.pdf
        f"https://knowthefactsmmj.com/wp-content/uploads/ommu_updates/{year}/{date_part}{year_short}-OMMU-Weekly-Update.pdf",  # MMDDYY format with -OMMU-Weekly-Update.pdf
        f"https://knowthefactsmmj.com/wp-content/uploads/ommu_updates/{year}/{date_part}-OMMU-Update.pdf",  # MMDD format with -OMMU-Update.pdf
        f"https://knowthefactsmmj.com/wp-content/uploads/ommu_updates/{year}/{date_part}-OMMU-Weekly-Update.pdf"  # MMDD format with -OMMU-Weekly-Update.pdf
    ]
    ...
    for url in url_formats:
        ...
        if pdf_file:
            return pdf_file  # Return the first successful download

    print(f"Failed to download PDF for {report_date_str} after trying all URL formats.")
    return None  # If none of the formats worked
...
def process_single_report(report_date_str):
    pdf_file = try_download_with_fallback(report_date_str)

    if pdf_file:
        pdf_text = extract_text_from_pdf(pdf_file)
        headers, table_data = extract_table_from_text(pdf_text)
        if table_data:
            return process_data(table_data, headers, report_date_str)
        else:
            print("No data extracted from PDF")
            return None
    else:
        print(f"Failed to download PDF for {report_date_str}")
        return None
...
if __name__ == "__main__":
    ...
    for report_date_str in valid_dates:
        ...
        processed_data = process_single_report(report_date_str)
